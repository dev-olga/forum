...
urlpatterns = patterns(''
    ...
    , url(r'^thread/(?P<id>\d+)/check_new_posts/last_loaded=(?P<last_loaded>\d+)/$', views.CheckNewPostsView.as_view(),
        name='check_new_posts')
    , url(r'^thread/(?P<id>\d+)/post/(?P<post_id>\d+)/edit/$', views.EditPostView.as_view(),
        name='edit_post')
